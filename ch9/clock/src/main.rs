...
const LOCAL_ADDR: &'static str = "0.0.0.0:12300"; // 12300は規定ポート番号
...
impl NTPMessage {
    ...
    fn client() -> Self {
        ...
        msg.data[0] |= VERSION;
        msg.data[0] |= MODE;
        msg // msg.data[0]は 001_1011と等しくなる(10進数の27)
    }

    fn parse_timestamp(&self, i: usize) -> Result<NTPTimestamp, std::io::Error> {
        let mut reader = &self.data[i..i + 8]; // 最初のバイトへのスライスを作る
...
fn check_time() -> Result<f64, std::io::Error> {
    ...
    for &server in servers.iter() {
        ...
        match calc {
            Ok(time) => {
                println!("{}ms away from locla system time", time.offset());
                times.push(time);
            }
